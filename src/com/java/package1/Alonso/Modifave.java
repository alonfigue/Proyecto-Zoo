...
public class Modifave extends JFrame {
	 ...
	 * @throws SQLException 
	 */
	public Modifave(ResultSet rs, int id) throws SQLException {

        Avestruz naves = new Avestruz();
        
		
		while (rs.next()) {
			naves.setNombreComun(rs.getString(2));
			naves.setNombreCientifico(rs.getString(3));
			naves.setGenero(rs.getString(4));
			naves.setEdad(rs.getInt(5));
			naves.setPeso(rs.getInt(6));
			naves.setPeriodoDeIncubacion(rs.getInt(7));
			naves.setVuela(rs.getBoolean(8));
			naves.setTamanoCuello(rs.getDouble(9));
		...
		if(naves.getVuela()==true) {
			vuela.setSelected(true);
		}
		vuela.setBounds(218, 240, 86, 20);
		contentPane.add(vuela);
		
		cuello = new JTextField(String.valueOf(naves.getTamanoCuello()));
		...
		if("macho".equals(naves.getGenero())) {
		...
		if("hembra".equals(naves.getGenero())) {
		...
		btnAgregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				db.dbPrepareStatementtz("UPDATE avestruz SET \"periodoDeIncubacion\"=?, vuela=?, \"tamanoCuello\"=? WHERE id="+id+";", ste);
				//********         
		...
		btnAgregar.setBounds(77, 302, 148, 34);
		contentPane.add(btnAgregar);
		
		lblNewLabel_1 = new JLabel("Avestruz:");
		...
		btnNewButton.setBounds(235, 302, 114, 34);
		contentPane.add(btnNewButton);
		
		textField = new JTextField(String.valueOf(id));
		textField.setEditable(false);
		textField.setBounds(136, 21, 86, 20);
